...
interface PokemonCardProps {
  ...
  setNewPokemonForLike?: any;
  setNewPokemonForBookmark?: any;
}

function PokemonCard({ pokemon, setNewPokemonForLike, setNewPokemonForBookmark }: PokemonCardProps) {
  const [isImageLoading, setImageLoading] = React.useState(true);

  const newpokemonFromStore = useStore((state) => state.newpokemon);
  ...
  const testAddNewPokemonForLike = (newpokemonname: string) => {
    if (!useremail) {
      ...
      if (userDetails && userDetails?.liked?.some((item: any) => String(item) === newpokemonname)) {
        setNewPokemonForLike((prev: any) => prev.filter((item: {name: string}) => item.name !== newpokemonname));
        toast.error(`Покемон ${newpokemonname} удален из понравившехся`, {
          icon: '❌'
        });
      } else {
        setNewPokemonForLike((prev: {name: string}[]) => [...prev, {name: newpokemonname}]);
  ...
  return (
    <div ref={ref} id="pockemon-card" data-name={pokemon?.name} className="relative p-4 bg-white rounded-xl shadow-sm flex flex-col gap-2">
